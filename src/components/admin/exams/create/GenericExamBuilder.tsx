...
export default function GenericExamBuilder({ mode, category, examId, initial, saveConfig }: GenericExamBuilderProps) {
  ...
  const [sections, setSections] = useState<Section[]>(() => {
    if (initial?.sections) return initial.sections;
    if (category === "IELTS") return createIELTSSections();
    return [];
  });
  ...
  const saveQuestion = () => {
    ...
    const isExisting = (id: string) =>
      currentSection.questions.find((q: Question) => q.id === id) !== undefined;
    ...
    const updatedSections = sections.map((s: Section) => {
        ...
        return {
          ...
          subsections: s.subsections.map((sub: Section) =>
            sub.id === currentSection.id
              ? {
                ...sub,
                questions: sub.questions.find((q: Question) => q.id === editingQuestion.id)
      ...
      return s.id === currentSection?.id
        ? {
          ...s,
          questions: isExisting(editingQuestion.id)
    ...
    const findCurrentSection = (list: Section[]): Section | null => {
  ...
  const deleteQuestion = (questionId: string) => {
    ...
    if (currentSection?.isSubsection) {
      const subsection = sections
        .find((s) => s.subsections?.some((sub) => sub.id === currentSection.id))
        ?.subsections?.find((sub) => sub.id === currentSection.id);
      if (subsection) {
    ...
    const questionText = questionToDelete
      ? getPromptDisplayText(questionToDelete.prompt) || "Question"
      : undefined;
        ...
        <div className="bg-white border-2 border-slate-900 rounded-md p-4 sm:p-6 mb-6 relative">
          ...
          <div className="absolute top-4 right-4 sm:top-6 sm:right-6 flex items-center gap-2">
            <div className="flex items-center gap-0.5">
              <span className="w-1.5 h-1.5 bg-slate-900 rounded-full" style={{ animation: 'bounce-slow 1.2s infinite', animationDelay: '0s' }}></span>
              <span className="w-1.5 h-1.5 bg-slate-900 rounded-full" style={{ animation: 'bounce-slow 1.2s infinite', animationDelay: '0.2s' }}></span>
              <span className="w-1.5 h-1.5 bg-slate-900 rounded-full" style={{ animation: 'bounce-slow 1.2s infinite', animationDelay: '0.4s' }}></span>
            </div>
