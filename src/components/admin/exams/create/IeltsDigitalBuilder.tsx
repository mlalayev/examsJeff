...
export function prepareIELTSSections(): Section[] {
...
export interface IeltsDigitalBuilderInitial {
  title: string;
  track: string;
...
interface IeltsDigitalBuilderProps {
  mode: "create" | "edit";
  examId?: string;
  initial?: IeltsDigitalBuilderInitial;
}

export default function IeltsDigitalBuilder({ mode, examId, initial }: IeltsDigitalBuilderProps) {
  const router = useRouter();

  const [examTitle, setExamTitle] = useState(initial?.title ?? "IELTS — New Exam");
  ...
  const [sections, setSections] = useState<Section[]>(
    () => initial?.sections ?? prepareIELTSSections()
  );
  ...
  const saveExam = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(json.error || "Failed to save IELTS exam");
      const savedId = json.exam?.id || json.examId || examId || "";
      router.push(`/dashboard/admin/exams/${savedId}`);
    } catch (e) {
      showAlert("Save failed", e instanceof Error ? e.message : "Failed to save IELTS exam", "error");
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="mb-6 sm:mb-10 flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="text-xl sm:text-2xl font-medium text-gray-900">
            {isEdit ? "Edit IELTS Exam" : "Create IELTS Exam"}
          </h1>
